...
public class SeatView extends ImageView {
	...
	// 原始盒子大小
	int originalBoxSize = 60;
	// 原始位置大小
	int originalSeatSize = 50;
	...
	private void init() {
		...
		column = COL;
        //已售
		normalSeatSelected = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.seat_yx);
		 //锁定
		normalSeatSell = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.seat_sd);
		//可选
		normalSeatOptionSelect = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.seat_kx);
		// normalSeatLock =
